...
func _ready() -> void:
	...
	_button.tooltip_text = "Chest of Love Notes"
...
func _process(delta: float) -> void:
	...
	if chest_state == ChestState.AVAILABLE or chest_state == ChestState.READY:
		...
		if chest_state == ChestState.READY:
...
func apply_ready_idle_state() -> void:
	## Restore the ready multi-scroll chest idle without replaying message flow.
	animating = false
	_skip = false
	_idle_time = 0.0
	_press_scale = 1.0
