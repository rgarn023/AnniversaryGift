...
func _test_build_version() -> void:
	...
	_assert(
		preset.contains("ChestOfLoveNotes-game-quality-chest-disconnect-fix-debug.apk")
		or preset.contains("splash-timing-chest-animation-fix-debug.apk"),
		"export APK name"
	)
	_assert(preset.contains("version/code=33") or preset.contains("version/code=32"), "export versionCode recent")
