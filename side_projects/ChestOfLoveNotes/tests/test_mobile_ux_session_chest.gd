...
func _test_startup_charoite() -> void:
	...
	_assert(boot_src.contains("MIN_DURATION_SEC := 1.75"), "boot minimum ~1.75 seconds")
	...
	_assert(boot_src.contains("_trim_near_black_margins"), "display trims logo letterbox in memory")
...
func _test_build_version() -> void:
	...
	_assert(preset.contains("ChestOfLoveNotes-map-currentlocation-schedule-selfsend-fixes-debug.apk"), "export APK name")
	_assert(preset.contains("version/code=23"), "export versionCode 23")
