...
func _run() -> void:
	...
	_assert(not loc_kt.contains("Tasks.await"), "no Tasks.await deadlock")
	_assert(loc.contains("MAX_ACCEPTABLE_ACCURACY_M := 500.0"), "sensible accuracy threshold 500m")
	_assert(loc.contains("MAX_FIX_AGE_MS := 180000") or loc.contains("MAX_FIX_AGE_MS := 120000"), "sensible fix age for selection")
	_assert(loc.contains("_active_request_token") or loc.contains("keeping settled success"), "GDScript timeout race guard")
	_assert(loc.contains("recent_cached") or loc.contains("trying last-known"), "timeout tries last-known")
	...
	_assert(preset.contains("version/code=28"), "export versionCode 28")
	_assert(preset.contains("current-location-qr-camera-fix-debug.apk"), "export APK name")
	_assert(gitignore.contains("ChestOfLoveNotes-current-location-qr-camera-fix-debug.apk"), "APK gitignore allow")
