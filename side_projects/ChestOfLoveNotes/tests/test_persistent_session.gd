...
func _test_demo_disabled_online_build() -> void:
	...
	_assert(preset.contains("ChestOfLoveNotes-android-permissions-fix-debug.apk") or preset.contains("preview-radius-attachments") or preset.contains("chest-location-lock") or preset.contains("secure-session"), "export targets COLN APK")
	_assert(preset.contains("version/code=26") or preset.contains("version/code=20") or preset.contains("version/code=17") or preset.contains("version/code=15"), "versionCode bumped")
