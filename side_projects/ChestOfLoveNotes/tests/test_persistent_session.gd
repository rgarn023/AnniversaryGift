...
func _test_demo_disabled_online_build() -> void:
	...
	_assert(preset.contains("ChestOfLoveNotes-chest-location-lock-debug.apk") or preset.contains("profile-compose-sent-chest-fixes") or preset.contains("targeted-fixes") or preset.contains("secure-session"), "export targets COLN APK")
	_assert(preset.contains("version/code=15") or preset.contains("version/code=13") or preset.contains("version/code=12"), "versionCode bumped")
...
func _test_anniversary_gift_untouched() -> void:
	...
	_assert(BuildFlags.APP_VERSION_CODE >= 15, "COLN versionCode incremented for location/performance fixes")
