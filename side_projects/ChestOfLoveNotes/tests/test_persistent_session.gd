...
func _test_demo_disabled_online_build() -> void:
	...
	_assert(preset.contains("version/code=9") or preset.contains("version/code=8"), "versionCode bumped")
...
func _test_anniversary_gift_untouched() -> void:
	...
	_assert(BuildFlags.APP_VERSION_CODE >= 9, "COLN versionCode incremented for mobile correction")
