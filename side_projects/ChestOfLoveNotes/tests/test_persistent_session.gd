...
func _test_demo_disabled_online_build() -> void:
	...
	_assert(preset.contains("ChestOfLoveNotes-send-map-preview-picker-fixes-debug.apk") or preset.contains("preview-radius-attachments") or preset.contains("chest-location-lock") or preset.contains("secure-session"), "export targets COLN APK")
	_assert(preset.contains("version/code=18") or preset.contains("version/code=17") or preset.contains("version/code=15"), "versionCode bumped")
...
func _test_anniversary_gift_untouched() -> void:
	...
	_assert(BuildFlags.APP_VERSION_CODE >= 18, "COLN versionCode incremented for send/map/preview/picker")
