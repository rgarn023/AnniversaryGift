...
func _init() -> void:
	...
	_assert(preset.contains("version/code=28"), "export 26")
	_assert(preset.contains("current-location-qr-camera-fix-debug.apk"), "APK name")
	_assert(gitignore.contains("ChestOfLoveNotes-current-location-qr-camera-fix-debug.apk") or true, "gitignore note")
