...
func _run() -> void:
	...
	_assert(flags.contains("APP_VERSION_CODE := 24"), "versionCode 24")
	_assert(preset.contains("version/code=24"), "export 24")
	_assert(preset.contains("my-person-qr-notifications-debug.apk"), "APK name")
	_assert(preset.contains("permissions/camera=true"), "camera export permission")
	_assert(gitignore.contains("ChestOfLoveNotes-my-person-qr-notifications-debug.apk"), "APK gitignore allow")
