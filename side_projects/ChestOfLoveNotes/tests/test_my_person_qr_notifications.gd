...
func _run() -> void:
	...
	_assert(preset.contains("permissions-map-chest-notification-performance-debug.apk"), "APK name")
	_assert(preset.contains("permissions/camera=true"), "camera export permission")
	_assert(gitignore.contains("ChestOfLoveNotes-permissions-map-chest-notification-performance-debug.apk"), "APK gitignore allow")
