extends SceneTree
## Headless visual validation for v49 grounding / scroll / glow / ocean glisten.
...
func _run() -> void:
	var out_dir := "/tmp/chest_audit_v49/runtime"
	DirAccess.make_dir_recursive_absolute(out_dir)
	DirAccess.make_dir_recursive_absolute("/opt/cursor/artifacts/chest_v49_validation")
	...
	## Advance glisten animation a bit so capture shows the sheen layer.
	for _i in range(30):
		await process_frame
	...
	var shots := [
		...
		{"name": "04_scroll_hidden", "p": LoveNotesChest.SCROLL_REVEAL_START_PROGRESS, "scroll": true},
		{"name": "05_scroll_first_peek", "p": 0.52, "scroll": true, "rise": 0.05},
		{"name": "06_scroll_25", "p": 0.66, "scroll": true, "rise": 0.25},
		{"name": "07_scroll_50", "p": 0.78, "scroll": true, "rise": 0.50},
		{"name": "08_scroll_65", "p": 0.90, "scroll": true, "rise": 0.72},
		...
		{"name": "12_ocean_glisten", "p": 0.0, "scroll": false},
	]
	for s in shots:
		...
		if bool(s["scroll"]):
			...
			elif float(s["p"]) > LoveNotesChest.SCROLL_REVEAL_START_PROGRESS + 0.001:
		...
		if img:
			...
			img.save_png(path)
			img.save_png("/opt/cursor/artifacts/chest_v49_validation/%s.png" % s["name"])
			print("WROTE ", path)
		var tex_path := ""
	...
	## Ocean glisten band bounds (water only).
	...
	print("SCROLL_DISPLAY_SCALE=", LoveNotesChest.SCROLL_DISPLAY_SCALE)
