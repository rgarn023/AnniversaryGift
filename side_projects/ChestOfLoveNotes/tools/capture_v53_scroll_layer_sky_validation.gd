extends SceneTree
## Headless visual validation for v53 scroll layer + sky polish.
...
func _run() -> void:
	var out_dir := "/tmp/chest_audit_v53/runtime"
	DirAccess.make_dir_recursive_absolute(out_dir)
	DirAccess.make_dir_recursive_absolute("/opt/cursor/artifacts/chest_v53_validation")
	...
	for s in shots:
		...
		if img:
			...
			img.save_png(path)
			img.save_png("/opt/cursor/artifacts/chest_v53_validation/%s.png" % s["name"])
			print("WROTE ", path)
		var tex_path := ""
	...
	for tod in tod_hours:
		...
		if img2:
			...
			img2.save_png("/opt/cursor/artifacts/chest_v53_validation/%s.png" % tod["name"])
