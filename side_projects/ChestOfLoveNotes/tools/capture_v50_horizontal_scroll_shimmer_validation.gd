extends SceneTree
## Headless visual validation for v50 horizontal scroll + water shimmer.
...
func _run() -> void:
	var out_dir := "/tmp/chest_audit_v50/runtime"
	DirAccess.make_dir_recursive_absolute(out_dir)
	DirAccess.make_dir_recursive_absolute("/opt/cursor/artifacts/chest_v50_validation")
	...
	for s in shots:
		...
		if img:
			...
			img.save_png(path)
			img.save_png("/opt/cursor/artifacts/chest_v50_validation/%s.png" % s["name"])
			print("WROTE ", path)
		var tex_path := ""
