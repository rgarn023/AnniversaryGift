extends SceneTree
## Headless visual validation for v55 scroll depth + top sky + local-time fix.
...
func _run() -> void:
	var out_dir := "/tmp/chest_audit_v55/runtime"
	DirAccess.make_dir_recursive_absolute(out_dir)
	DirAccess.make_dir_recursive_absolute("/opt/cursor/artifacts/chest_v55_validation")
	...
	for s in shots:
		...
		if img:
			...
			img.save_png(path)
			img.save_png("/opt/cursor/artifacts/chest_v55_validation/%s.png" % s["name"])
			print("WROTE ", path)
		var tex_path := ""
	...
	for tod in tod_hours:
		...
		if img2:
			...
			img2.save_png("/opt/cursor/artifacts/chest_v55_validation/%s.png" % tod["name"])
	...
	print("SKY_Y=", env._sky_gradient_view.position.y if env._sky_gradient_view else -1.0)
